import{G as s}from"./vendor-core-C3XopWI3.js";import"./vendor-react-UlIMTZ8g.js";const l="AIzaSyA5QqOGhud-GOyCdokpneK3wo3F-e7kP88";let a=null;try{a=new s(l)}catch(e){console.error("Error al inicializar GoogleGenerativeAI:",e)}const i=async(e="gemini-1.5-flash",n)=>{if(!a)return console.error("GoogleGenerativeAI no está inicializado. Verifica tu API Key."),null;try{return(await(await a.getGenerativeModel({model:e}).generateContent(n)).response).text()}catch(r){return console.error(`Error generando contenido con ${e}:`,r),null}},d=async(e,n)=>{const r=`
    ...
    `;try{const o=await i("gemini-1.5-flash",r);if(!o)return null;console.log("Raw AI Response (RCA):",o);const t=o.match(/\{[\s\S]*\}/);return t?JSON.parse(t[0]):(console.error("No JSON found in response"),null)}catch(o){return console.error("Error in analyzeRootCause:",o),null}};export{d as analyzeRootCause,i as generateContent};
