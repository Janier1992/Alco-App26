import{G as l}from"./vendor-core-BGJXmmY_.js";import"./vendor-react-BdtBXqdl.js";const i="AIzaSyAao48NBXbU0Kqwp8vJJl7J3I4vU4sRKO4";let t=null;try{t=new l(i)}catch(n){console.error("Error al inicializar GoogleGenerativeAI:",n)}const s=async(n="gemini-1.5-flash",r)=>{if(!t)return console.error("GoogleGenerativeAI no está inicializado. Verifica tu API Key."),null;try{return(await(await t.getGenerativeModel({model:n}).generateContent(r)).response).text()}catch(e){return console.error(`Error generando contenido con ${n}:`,e),null}},d=async(n,r)=>{const e=`
    ...
    `;try{const o=await s("gemini-1.5-flash",e);if(!o)return null;console.log("Raw AI Response (RCA):",o);const a=o.match(/\{[\s\S]*\}/);return a?JSON.parse(a[0]):(console.error("No JSON found in response"),null)}catch(o){return console.error("Error in analyzeRootCause:",o),null}},p=async n=>{const r=`
    ...
    `;try{const e=await s("gemini-1.5-flash",r);if(!e)return null;console.log("Raw AI Response (Claim):",e);const o=e.match(/\{[\s\S]*\}/);return o?JSON.parse(o[0]):(console.error("No JSON found in response"),null)}catch(e){return console.error("Error in analyzeClaim:",e),null}};export{p as analyzeClaim,d as analyzeRootCause,s as generateContent};
