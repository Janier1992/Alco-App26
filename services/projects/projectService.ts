...
export interface ProjectService {
    ...
    updateChecklistItem(itemId: string, updates: Partial<any>): Promise<any>;
    ...
    archiveTask(taskId: string): Promise<any>;
...
export const projectService: ProjectService = {
    ...
    async getTasksWithDetails(columnIds: string[]) {
        try {
            const { data, error } = await supabase
                .from('board_tasks')
                .select('*, labels:task_labels(*), assignedUsers:task_assignees(*), checklist:task_checklists(*), attachments:task_attachments(*), comments:task_comments(*)')
            ...
            console.warn('Advanced schema missing (is_archived, task_checklists). Falling back to basic query.', err);
            // Fallback for missing is_archived and task_checklists
            const { data, error } = await supabase
                .from('board_tasks')
                .select('*, labels:task_labels(*), assignedUsers:task_assignees(*), attachments:task_attachments(*), comments:task_comments(*)')
                .in('column_id', columnIds)
                .order('position');
    ...
    async archiveTask(taskId) {
        // Here we could set a flag or move to an `archived` column. Assume simple update for now.
        const { error } = await supabase.from('board_tasks').update({ is_archived: true }).eq('id', taskId);
        if (error) throw error;
    }
};
