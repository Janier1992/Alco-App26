...
const NonConformities: React.FC = () => {
    ...
    const handleCreateNC = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            await EmailService.send({
                to: 'calidad@alco.com',
                subject: `Nueva No Conformidad Detectada: ${newNCData.title}`,
                body: `Se ha registrado una nueva NC en el proyecto ${newNCData.project}.\nPrioridad: ${newNCData.severity}\nID: ${data.id}`,
                moduleName: 'non-conformities',
                referenceId: data.id,
                triggeredBy: 'system'
            });
    ...
    const handleUpdateNC = async (updated: any) => {
        ...
        try {
            ...
            await EmailService.send({
                to: 'calidad@alco.com',
                subject: `Estado de NC Actualizado: ${updated.title}`,
                body: `La No Conformidad ha cambiado a estado: ${updated.status}.\nID: ${db_id}`,
                moduleName: 'non-conformities',
                referenceId: db_id,
                triggeredBy: 'system'
            });
    ...
    const inputStyles = "w-full p-4 bg-slate-50 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/[0.06] rounded-2xl text-xs font-bold text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500/40 outline-none uppercase transition-all placeholder:text-slate-400";
        ...
        <>
            <div className="animate-fade-in space-y-10 pb-20">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <button onClick={() => setIsCreateModalOpen(true)} className="w-full md:w-auto px-10 py-5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3">
                        <PlusIcon /> Abrir Nuevo Hallazgo
                    </button>
                </div>

                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex-grow md:flex-initial flex items-center gap-3 px-6 py-4 bg-white dark:bg-white/[0.03] rounded-[2rem] border border-slate-100 dark:border-white/[0.06] shadow-sm focus-within:ring-2 focus-within:ring-indigo-500/40 transition-all">
                    ...
                    {loading ? (
                        ...
                        <div key={nc.id} className="premium-card p-6 space-y-4 shadow-sm">
                            <div className="flex justify-between items-start">
                                <span className="text-[10px] text-indigo-500 font-black uppercase tracking-widest bg-indigo-500/10 px-3 py-1 rounded-full">{nc.id}</span>
                                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${nc.status === 'Cerrada' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20'}`}>{nc.status}</span>
                            </div>
                            <div>
                                ...
                                <p className="text-[10px] text-slate-400 font-bold uppercase mt-1.5 tracking-wider flex items-center gap-1">
                                    <span className="w-1 h-1 rounded-full bg-indigo-500"></span> {nc.process}
                                </p>
                            </div>
                            <div className="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-white/5">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => { setSelectedNC(nc); setIsActionModalOpen(true); }} className="p-3 bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 rounded-2xl hover:text-indigo-500 transition-all"><BrainIcon /></button>
                                    ...
                                    <tr key={nc.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-all group">
                                        ...
                                        <td className="px-10 py-8 text-right">
                                            <div className="flex justify-end gap-3">
                                                <button onClick={() => { setSelectedNC(nc); setIsActionModalOpen(true); }} className="p-3 bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-slate-400 rounded-2xl hover:text-indigo-500 transition-all shadow-sm border border-slate-200 dark:border-white/5" title="Analizar Hallazgo"><BrainIcon /></button>
