...
const Library: React.FC = () => {
    ...
    const handleDelete = async (id: string) => {
        const confirmed = await confirm({
            ...
            message: '¿Desea eliminar permanentemente este documento del repositorio oficial?',
            variant: 'danger',
    ...
    const inputStyles = "w-full p-4 bg-slate-50 dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/[0.06] rounded-2xl text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500/40 outline-none uppercase transition-all placeholder:text-slate-400";
        ...
        <div className="space-y-8 animate-fade-in pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    <button onClick={() => fetchRecords()} className="p-4 bg-white dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/[0.06] rounded-2xl text-slate-400 hover:text-indigo-500 transition-all shadow-sm">
                        <RefreshIcon />
                    </button>
                    <button
                        onClick={handleUploadClick}
                        className="flex-grow md:flex-none px-8 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-500/20 hover:scale-105 active:scale-95 flex items-center justify-center gap-2"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-1 gap-2">
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={`flex items-center justify-between p-3.5 rounded-2xl transition-all group ${selectedCategory === cat.id ? 'bg-indigo-500/10 text-indigo-500' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-white/5'}`}
                                >
                                    <div className="flex items-center gap-3 truncate">
                                        <i className={`${cat.icon} text-sm ${selectedCategory === cat.id ? 'text-indigo-500' : 'text-slate-400'}`}></i>
                                        <span className="text-[10px] font-black uppercase tracking-tight truncate">{cat.label}</span>
                                    </div>
                                    <span className={`hidden sm:inline text-[9px] font-bold px-2 py-0.5 rounded-full ${selectedCategory === cat.id ? 'bg-indigo-500/20 text-indigo-500' : 'bg-slate-100 dark:bg-white/5 text-slate-400'}`}>
                                        {cat.count}
                                    </span>
                                </button>
                            ))}
                ...
                <div className="lg:col-span-9 space-y-6">
                    <div className="relative group">
                        <input
                            className="w-full pl-14 pr-6 py-5 bg-white dark:bg-white/[0.03] border border-slate-200/80 dark:border-white/[0.06] rounded-[2.5rem] shadow-sm text-sm font-bold text-slate-800 dark:text-white focus:ring-2 focus:ring-indigo-500/40 transition-all outline-none"
                            placeholder="Buscar por código (ej: PL-TH) o nombre de documento..."
                            value={searchTerm}
                            onChange={e => setSearchTerm(e.target.value)}
                        />
                        <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-hover:text-indigo-500 transition-colors text-xl">
                            <SearchIcon />
                        </div>
                    </div>
                    ...
                    <div className="premium-card overflow-hidden animate-fade-in shadow-xl">
                        {isLoading ? (
                            <div className="p-20 text-center space-y-4">
                                <div className="animate-spin text-4xl text-indigo-500 flex justify-center"><RefreshIcon /></div>
            ...
            {isUploadModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
                    <div className="bg-white dark:bg-alco-dark w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-white/10 animate-scale-in">
                        <div className="p-8 border-b border-slate-100 dark:border-white/[0.06] flex justify-between items-center bg-slate-50 dark:bg-white/[0.02]">
                            <div>
                                <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Cargar Nuevo <span className="text-indigo-500">Documento</span></h3>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Biblioteca SGC - Control ISO 9001</p>
                            </div>
                            <button onClick={() => setIsUploadModalOpen(false)} className="p-3 bg-white dark:bg-white/[0.03] rounded-2xl text-slate-400 hover:text-rose-500 transition-all border border-slate-200/80 dark:border-white/[0.06] shadow-sm"><XCircleIcon /></button>
                        </div>
                        <form onSubmit={handleSaveDocument} className="p-8 space-y-6">
                            ...
                            <div className="pt-4">
                                ...
                                <div
                                    ...
                                    className={`border-2 border-dashed rounded-3xl p-10 text-center cursor-pointer transition-all ${tempFile ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/5' : 'border-slate-200 dark:border-white/10 hover:border-indigo-500 hover:bg-slate-50 dark:hover:bg-white/5'}`}
                                >
                                    {tempFile ? (
                            ...
                            <button type="submit" className="w-full py-5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-3xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-indigo-500/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3">
            ...
            {docToView && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-fade-in">
                    <div className="bg-white dark:bg-alco-dark w-full h-full max-w-6xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-white/10 flex flex-col">
                        <div className="p-6 border-b border-slate-100 dark:border-white/[0.06] flex justify-between items-center bg-slate-50 dark:bg-white/[0.02]">
                            <div>
                            ...
                            <button onClick={() => setDocToView(null)} className="p-3 bg-white dark:bg-white/[0.03] rounded-2xl text-slate-400 hover:text-rose-500 transition-all border border-slate-200/80 dark:border-white/[0.06] shadow-sm"><XCircleIcon /></button>
                        </div>
                        ...
                        <div className="p-6 bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/[0.06] flex justify-end gap-3">
